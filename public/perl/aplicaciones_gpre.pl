...
while ( my $row = $sth->fetchrow_hashref ) {
	print "document.write('<tr><td>$row->{numinst}</td><td class=\"tdapli\" data-idapp=\"$row->{idapp}\">$row->{nomapp}</td><td>$row->{fabricante}</td><td>$row->{version}</td><td>$row->{carpeta}</td><td>$row->{comentarios}</td></tr>');\n";
}
