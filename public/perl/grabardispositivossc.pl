...
# TITLE:	 	grabardispositivossc.pl
# DESCRIPTION:	graba en la BD el dispositivo sin conexion entrado
# AUTHOR:		José Serena
# DATE:			07/09/2015
# VERSION:		4.0
...
# grabar los datos pasados en la tabla de dispositivos sin conexion
my $res;
#my $orden = "INSERT INTO dispositivossc VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )";
#my $sth = $dbh->prepare( $orden );
#if ( $datos{fechacompra} ) {
#	$res = $sth->execute( '', $datos{descripcion}, $datos{tipo}, $datos{ubicacion}, $datos{estado}, $datos{numserie},
#			    	$datos{responsable}, $datos{proveedor}, $datos{pedido}, $datos{factura}, $datos{fechacompra},
#			    	$datos{notas} );
#} else {
#	$res = $sth->execute( '', $datos{descripcion}, $datos{tipo}, $datos{ubicacion}, $datos{estado}, $datos{numserie},
#			    	$datos{responsable}, $datos{proveedor}, $datos{pedido}, $datos{factura}, NULL,
#			    	$datos{notas} );
#}
if ( $datos{fechacompra} ) {
	$res = $dbh->do("INSERT INTO dispositivossc VALUES ( '', '$datos{descripcion}', '$datos{tipo}', '$datos{ubicacion}', '$datos{estado}',
														'$datos{numserie}', '$datos{responsable}', '$datos{proveedor}', '$datos{pedido}',
														'$datos{factura}', '$datos{fechacompra}', '$datos{notas}' )");
} else {
	$res = $dbh->do("INSERT INTO dispositivossc VALUES ( '', '$datos{descripcion}', '$datos{tipo}', '$datos{ubicacion}', '$datos{estado}',
														'$datos{numserie}', '$datos{responsable}', '$datos{proveedor}', '$datos{pedido}',
														'$datos{factura}', NULL, '$datos{notas}' )");
}
...
if ( $res eq "0E0" ) {
	...
	print "\$( \"#resgrabar\" ).html( \"Dispositivo grabado\" );\n";
	print "\$( \"#resgrabar\" ).dialog( \"open\" );\n";
}
