...
# TITLE:	 	grabarproveedor.pl
# DESCRIPTION:	graba en la BD el proveedor entrado
# AUTHOR:		José Serena
# DATE:			07/09/2015
# VERSION:		4.0
...
my $res = $dbh->do("INSERT INTO proveedores VALUES ( '', '$datos{nombre}', '$datos{direccion}', '$datos{poblacion}', '$datos{provincia}',
											'$datos{cp}', '$datos{pais}', '$datos{nif}', '$datos{web}', '$datos{telefono1}', '$datos{telefono2}',
											'$datos{fax}', '$datos{correo}', '$datos{notas}' )");
											
if ( $res eq "0E0" ) {
	print "\$( \"#resgrabar\" ).html( \"Error al grabar el proveedor\" );\n";
	print "\$( \"#resgrabar\" ).dialog( \"open\" );\n";
} else {
	print "\$( \"#resgrabar\" ).html( \"Proveedor grabado\" );\n";
	print "\$( \"#resgrabar\" ).dialog( \"open\" );\n";
}
