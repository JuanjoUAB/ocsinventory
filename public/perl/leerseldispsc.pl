...
# TITLE:	 	leertiposdisp.pl
# DESCRIPTION:	lee los tipos de dispositivos y proveedores que se muestran como listas
# AUTHOR:		José Serena
# DATE:			07/09/2015
# VERSION:		4.0
...
$sth->execute();
while (@row = $sth->fetchrow_array) {
	print "\$(\"<option value='$row[0]'>$row[1]</option>\").appendTo(\"#lproveedor\")\n";
}
...
$sth->execute();
while (@row = $sth->fetchrow_array) {
	print "\$(\"<option value='$row[0]'>$row[1]</option>\").appendTo(\"#lestado\")\n";
}
