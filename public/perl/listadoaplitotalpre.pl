...
my $fila=2;
my $row;
my $orden="SELECT id,numinst,nomapp,idapp,fabricante,version,carpeta,comentarios FROM aplicacionesg";
my $sth = $dbh->prepare($orden);
$sth->execute();
while ($row = $sth->fetchrow_hashref) {
	
        $row->{nomapp}=decode_entities($row->{nomapp}); 
        $row->{fabricante}=decode_entities($row->{fabricante});
        $row->{version}=decode_entities($row->{version});
        $row->{carpeta}=decode_entities($row->{carpeta});
        $row->{comentarios}=decode_entities($row->{comentarios});
        
        $hoja1 -> write("A$fila", $row->{numinst});	
	$hoja1 -> write("B$fila", $row->{nomapp});
	$hoja1 -> write("C$fila", $row->{fabricante});
	$hoja1 -> write("D$fila", $row->{version});
	$hoja1 -> write("E$fila", $row->{carpeta});
	$hoja1 -> write("F$fila", $row->{comentarios});
