...
# DESCRIPTION:	lista por pantalla los dispositivos configurados
# AUTHOR:		José Serena
# DATE:			07/09/2015
# VERSION:		4.0
...
print "document.write('<th class=\"group-false\">Notas</th>');\n";
...
while (my $row = $sth->fetchrow_hashref) {
	...
	my $row2 = $sth2->fetchrow_hashref;
        ...
        $row2->{notas}=decode("UTF-8",$row2->{notas});
        $row2->{notas}=caresp($row2->{notas});
