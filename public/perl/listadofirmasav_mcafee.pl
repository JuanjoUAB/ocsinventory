...
# DESCRIPTION:	genera Excel con la version de firmas del antivirus McAfee
# AUTHOR:		José Serena
# DATE:			07/09/2015
# VERSION:		4.0
...
my @datos = ( "Nombre PC", 15, "Usuario", 12, "Centro", 12, "IP", 12, "Version AV", 15, "Version Motor", 15, "Version DAT", 15, "Fecha DAT", 15, "Dias", 5 );
...
while (@row = $sth->fetchrow_array) {
	...
	while (@row2 = $sth2->fetchrow_array) {
		if ( defined($row2[0]) ) {
			my $vav = $hvmcafee{$row2[0]} ? $hvmcafee{$row2[0]} : $row2[0];
			$hoja1 -> write("E$fila", $vav);
			last;
		}
	}
	...
	# fecha DAT McAfee
	my $fechaav = "";
	$orden="SELECT regvalue FROM registry WHERE hardware_id=$row[0] AND ( name='Firma Antivirus McAfee' OR name='Firma Antivirus McAfee64b' )";
	my $sth2 = $dbh->prepare($orden);
	$sth2->execute();
	
	@row2 = $sth2->fetchrow_array;
	my ($anyoav, $mesav, $diaav, $dias);
	if ( defined($row2[0]) ) {
		($anyoav, $mesav, $diaav) = $row2[0] =~ /(.*?)\/(.*?)\/(.*)/;
		$hoja1 -> write("H$fila", "$diaav/$mesav/$anyoav");
		($anyoav, $mesav, $diaav) = $row2[0] =~ /(.*?)\/(.*?)\/(.*)/;
		$mesav--;
		eval { timelocal(0, $min, $hora, $diaav, $mesav, $anyoav) }; # hay fechas erroneas que abortan el programa
		$fechaav = $@ ? 0 : timelocal(0, $min, $hora, $diaav, $mesav, $anyoav);

		$dias = sprintf("%.0f",(time-$fechaav)/86400);
		$hoja1 -> write("I$fila", $dias);		
	} else {
		$hoja1 -> write("H$fila", " ");
		$hoja1 -> write("I$fila", " ");
	}
