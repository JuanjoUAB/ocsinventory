...
# TITLE:	 	grabarprovmodif.pl
# DESCRIPTION:	graba en la BD el proveedor modificado
# AUTHOR:		José Serena
# DATE:			07/09/2015
# VERSION:		4.0
...
# grabar los datos pasados en la tabla de proveedores
my $res = $dbh->do("UPDATE proveedores SET nombre='$datos{nombre}', direccion='$datos{direccion}', poblacion='$datos{poblacion}', provincia='$datos{provincia}',
											codpostal='$datos{cp}', pais='$datos{pais}', nif='$datos{nif}', web='$datos{web}', telefono1='$datos{telefono1}', telefono2='$datos{telefono2}',
											fax='$datos{fax}', correo='$datos{correo}', notas='$datos{notas}' 
					WHERE ID='$datos{id}'");
											
if ( $res eq "0E0" ) {
	print "\$( \"#resgrabar\" ).html( \"Error al grabar el proveedor\" );\n";
