...
# TITLE:	 	aplicaciones_u.pl
# DESCRIPTION:	lista por pantalla las aplicaciones instaladas en el equipo seleccionado
# AUTHOR:		José Serena
# DATE:			07/09/2015
# VERSION:		4.0
...
print "document.write('<table id=\"taplicaciones\" class=\"tablesorter-blue\">');\n";
...
print "document.write('<th>Versión</th>');\n";
...
my $orden="SELECT publisher,name,version,folder,comments FROM softwares where hardware_id='$id'";
my $sth = $dbh->prepare($orden);
$sth->execute();
while ( my $row = $sth->fetchrow_hashref ) {
	my $nombre = $row->{publisher} ? $row->{publisher} : "ND";
	$nombre = caresp($nombre);
	my $fab = $row->{name} ? $row->{name} : "&nbsp;";
	$fab = caresp($fab);
	...
	print "document.write('<tr><td>$fab</td><td>$nombre</td><td>$version</td><td>$carpeta</td><td>$comentario</td></tr>');\n";
}
