...
# TITLE:	 	registrardispdesc.pl
# DESCRIPTION:	seleccionar masivamente los equipos desconocidos y registrarlos como conocidos
# AUTHOR:		José Serena
# DATE:			07/09/2015
# VERSION:		4.0
...
print "document.write('<th class=\"group-false filter-false sorter-false\">Sel</th>');\n";
...
my $orden="SELECT id.mac,id.os_nombre,id.os_familia,id.os_fabricante,id.os_tipo,id.sysDescr,id.sysName,id.nbdominio,id.nbnombre,np.ip,np.date,np.name FROM identificaciondisp AS id,netmap AS np WHERE id.mac=np.mac AND id.conocido='0'";
my $sth = $dbh->prepare($orden);
$sth->execute();
while (my $row = $sth->fetchrow_hashref) {
	...
	print "document.write('<tr>".
	"<td><button eb=0 class=\"bsel\"><img eb=0 src=\"imagenes/pc_off.png\" alt=\"\" ></button></td>".
	"<td>$row->{mac}</td>".
	"<td>$fabricante</td>".
	"<td>$centro</td>".
	"<td>$row->{ip}</td>".
