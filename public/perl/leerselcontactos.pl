...
# TITLE:	 	leerselcontactos.pl
# DESCRIPTION:	lee los tipos de contacto y proveedores que se muestran como listas
# AUTHOR:		José Serena
# DATE:			07/09/2015
# VERSION:		4.0
...
while (@row = $sth->fetchrow_array) {
	print "\$(\"<option value='$row[0]'>$row[1]</option>\").appendTo(\"#tipocontacto\")\n";
}
...
$sth->execute();
while (@row = $sth->fetchrow_array) {
	print "\$(\"<option value='$row[0]'>$row[1]</option>\").appendTo(\"#lempresa\")\n";
}
