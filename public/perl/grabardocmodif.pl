...
# TITLE:	 	grabardocmodif.pl
# DESCRIPTION:	graba en la BD el documento modificado
# AUTHOR:		José Serena
# DATE:			07/09/2015
# VERSION:		4.0
...
my $res;
if ( $datos{fechadoc} ) {
	$res = $dbh->do("UPDATE documentos SET tipodoc='$datos{tipodoc}', proveedor='$datos{proveedor}', referencia='$datos{referencia}',
													fechadoc='$datos{fechadoc}', nfichero='$datos{nfichero}', notas='$datos{notas}'
					WHERE ID='$datos{id}'");
} else {
	$res = $dbh->do("UPDATE documentos SET tipodoc='$datos{tipodoc}', proveedor='$datos{proveedor}', referencia='$datos{referencia}',
													fechadoc=NULL, nfichero='$datos{nfichero}', notas='$datos{notas}'
					WHERE ID='$datos{id}'");
}
...
if ( $res eq "0E0" ) {
	...
	print "\$( \"#mensajegrabar\" ).html( \"Documento modificado\" );\n";
}
...
if ( -e $dirdocs.$datos{nfichero} ) {
	...
	if ( $datos{nfichero} =~ /\./ ) {
		($nombrerealfic, $extfic) = $datos{nfichero} =~ /(.*)\.(.*)/;
		$nombrerealfic .= "_$datos{id}.$extfic";
	} else {
		$nombrerealfic = $datos{nfichero}."_$datos{id}";
	}
	rename $dirdocs.$datos{nfichero},$dirdocs.$nombrerealfic;
