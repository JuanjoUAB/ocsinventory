...
# DESCRIPTION:	muestra los equipos y permite lanzar el control remoto RealVNC
# AUTHOR:		José Serena
# DATE:			07/09/2015
# VERSION:		4.0
...
while ($row = $sth->fetchrow_hashref) {
	...
	# comprobar si tienen RealVNC instalado
	$orden="SELECT name FROM softwares where hardware_id=$row->{id} and name like 'VNC %'";
	...
	if ( $uvnc ) {
		print "document.write('<tr>".
			"<td class=\"tdpc\" onclick=lanzarClienteRVNC(\"$nombre\",\"$row->{ipaddr}\")>$nombre</td>".
